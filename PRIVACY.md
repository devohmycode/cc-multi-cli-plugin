...
**The plugin has no hosted service or author-operated telemetry.** Its companion
and experimental gateway run on your machine; the gateway listens on localhost.
...
The current implementation keeps these local files. They may contain task content
or credentials; local storage does not mean the corresponding task content is
...
- **Setup records and CLI configuration** — the existing setup wizard records managed MCP servers in `managed-servers.json` alongside its plugin config, and can write MCP entries and backups in Codex's and Cursor's own configuration locations.
- **Workspace state** under `$CLAUDE_PLUGIN_DATA/state/<workspace-slug>-<hash>/` when that environment variable is set, otherwise under the system temporary directory's `codex-companion/<workspace-slug>-<hash>/`. `state.json` records configuration and tracked jobs; `jobs/<id>.json` contains job data and results. These support `/multi:status`, `/multi:result`, and `/multi:cancel`.
...
The gateway and harness refactor may change these paths. This policy describes
current storage, not a commitment to retain the old command implementation.

## What goes to third parties

Companion commands forward the task to the selected CLI. The experimental native
