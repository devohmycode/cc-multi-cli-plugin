...
const selected = process.argv[2]
  ? catalog.find((o) => o.model === process.argv[2] || o.worker === process.argv[2])
  : catalog.find((o) => o.selection.id.startsWith('composer-') && o.model.split('/').length === 3);
...
try {
  const request: MessagesRequest = {
    ...
    tools: [
      {
        name: 'Probe',
        description: 'Retrieve the test secret',
        input_schema: {
          type: 'object',
  ...
  const next: MessagesRequest = {
    ...
    messages: [
      ...
      {
        role: 'user',
        content: [{ type: 'tool_result', tool_use_id: call.id, content: `Secret: ${nonce}` }],
      },
    ],
  };
  ...
  const cancellable = new CursorBridge([selected], {
    cwd,
    createAgent: async (config) => {
      ...
      return {
        ...
        send: async (message, config) => {
          pausedRun = await agent.send(message, config);
          return pausedRun;
        },
      };
    },
  ...
  try {
    ...
    assert(pausedRun, 'SDK run was not created');
  ...
  const main = await run([
    ...
    '--',
    '-p',
    'Read fixture.txt, then use Edit to replace alpha with beta. Preserve the rest and report the complete resulting line.',
    '--model',
    selected.model,
    '--tools',
    'Read,Edit',
  ...
  const events = main.output
    .trim()
    .split('\n')
    .map((line) => JSON.parse(line));
  assert(
    events.some(
      (event) => event.type === 'result' && !event.is_error && event.result?.includes(nonce),
    ),
  );
  ...
  for (const model of ['sonnet', selected.model]) {
    const switched = await run([
      ...
      '--',
      '-p',
      'Without tools, repeat the exact full resulting line of fixture.txt from the earlier conversation.',
      '--resume',
      session,
      '--model',
      model,
    ...
    assert(
      switched.error.includes(
        model === 'sonnet' ? '"route":"anthropic","status":200' : '"route":"cursor"',
      ),
    );
  }
