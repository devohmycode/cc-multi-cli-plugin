...
// (transports/acp/client.mjs), its win32 spawn resolution (transports/acp/resolve.mjs), and the
...
import {
  ...
  listModelValueIds,
  runAcpTurn,
} from '../../plugins/multi/src/transports/acp/client.mjs';
import {
  pickLatestCursorVersionDir,
  resolveCursorAcp,
  resolveOpenCodeAcp,
} from '../../plugins/multi/src/transports/acp/resolve.mjs';
...
function tmpRecord() {
  return path.join(
    os.tmpdir(),
    `acp-rec-${process.pid}-${Math.random().toString(36).slice(2)}.jsonl`,
  );
}
...
test('cancel B (opencode mislabel): end_turn stopReason but cancelled=true', async () => {
  ...
  assert.equal(
    r.cancelled,
    true,
    'cancel-requested + turn-ended ⇒ cancelled regardless of stopReason',
  );
  assert.equal(r.stopReason, 'end_turn');
  assert.equal(r.error, null);
});
...
test('cwd containing spaces works end-to-end', async () => {
  ...
  try {
    const r = await runAcpTurn(spawnSpec(['--stream', 'spaced ok'], { cwd: dir }));
    assert.equal(r.error, null);
...
test('resolveModel callback: maps a requested name to a different live id, pinned via set_config', async () => {
  ...
  try {
    ...
    assert.equal(
      setConfig.set_config.value,
      'alpha[fast=true]',
      'the resolved composite id is pinned',
    );
  } finally {
    fs.rmSync(rec, { force: true });
  }
  ...
  try {
    const r = await runAcpTurn(
      spawnSpec(['--models', 'alpha[fast=true]', '--record', rec], {
        model: 'gamma',
        resolveModel: () => ({
          error: { code: 'config', message: 'gamma is ambiguous', detail: 'x, y' },
        }),
      }),
    );
    assert.ok(r.error, 'an error is set');
    ...
    assert.ok(
      !lines.some((l) => l.recv === 'session/prompt'),
      'no prompt sent when the resolver errors',
    );
  } finally {
    fs.rmSync(rec, { force: true });
  }
...
test('resolveOpenCodeAcp: discovers the platform exe under a fixture APPDATA', () => {
  ...
  try {
    ...
    assert.ok(r.exe?.endsWith('opencode.exe'));
...
test('resolveCursorAcp: win32 builds node.exe + index.js acp from the latest version dir', () => {
  ...
  try {
    ...
    assert.ok(r.exe?.endsWith('node.exe'));
...
test('vendor bundle exists, imports, exports the expected symbols, and records the locked SDK version', async () => {
  ...
  assert.ok(
    ...
    'the committed bundle must exist (run `npm run build:acp-vendor`)',
  );
