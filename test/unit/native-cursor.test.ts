...
const body: MessagesRequest = {
  model,
  messages: [{ role: 'user', content: 'Read then edit fixture.txt' }],
  tools: [
...
const calls = (r: MessagesResponse) =>
  r.content.filter(
    (b): b is Extract<ResponseContentBlock, { type: 'tool_use' }> => b.type === 'tool_use',
  );
function follow(
  request: MessagesRequest,
  response: MessagesResponse,
  text = 'Tool succeeded',
): MessagesRequest & { messages: RequestMessage[] } {
  assert(request.messages, 'Continuation requires a message history');
  return {
    ...request,
    messages: [
      ...request.messages,
      ...
      {
        role: 'user',
        content: calls(response).map((call) => ({
          type: 'tool_result',
          tool_use_id: call.id,
          content: text,
        })),
...
function customTools(config: AgentOptions) {
  assert(config.local?.customTools, 'Bridge must register its local tool callbacks');
  return config.local.customTools;
}
  ...
  const createAgent: CreateCursorAgent = async (config) => {
    ...
    return {
      ...
      send: async (message, sendOptions) => {
        ...
        const run: Run = {
          usage,
          id: `run-${configs.length}`,
          agentId: `agent-${configs.length}`,
          get status() {
            return status;
          },
          wait: () => result.promise,
          cancel: async () => {
            ...
            abort.abort();
            status = 'cancelled';
            result.resolve({ id: run.id, status });
          },
          async *stream() {},
          conversation: async () => [],
          supports: () => true,
        ...
        void Promise.resolve()
          ...
          .then(
            (text) => {
              status = 'finished';
              result.resolve({ id: run.id, status, result: text });
            },
            (error) => {
              status = 'error';
              result.resolve({ id: run.id, status, error: { message: String(error.message) } });
            },
          );
        return run;
      },
...
test('Cursor callback waits for Claude result, streams text, and replays HTTP retries without new runs', async (t) => {
  ...
  const fake = runtime(async (config, text) => {
    text('Reading.');
    readResult = await customTools(config).Read.execute(
      { file_path: 'fixture.txt' },
      { toolCallId: 'read' },
    );
    editResult = await customTools(config).Edit.execute(
  ...
  const local = fake.configs[0].local;
  assert(local);
  assert.deepEqual(local.settingSources, []);
  assert.equal(local.enableAgentRetries, false);
...
test('simultaneous retries share an in-flight request', async (t) => {
  ...
  const [one, two] = await Promise.all([
    bridge.handle(body, 'same', signal()),
    bridge.handle(body, 'same', signal()),
  ]);
...
test('late parallel callbacks are delivered once and retain their own results', async (t) => {
  ...
  const fake = runtime(async (config) => {
    const one = customTools(config).Read.execute({ file_path: 'a' }, { toolCallId: 'a' });
    await tick();
    const two = customTools(config).Read.execute({ file_path: 'b' }, { toolCallId: 'b' });
  ...
  assert.deepEqual(
    values,
    ['A', 'B'].map((text) => ({ content: [{ type: 'text', text }], isError: false })),
  );
});
...
test('permission denial is returned as an error; a different worker cannot settle the callback', async (t) => {
  let value: unknown;
  const fake = runtime(async (config) => {
    value = await customTools(config).Edit.execute({}, { toolCallId: 'edit' });
  ...
  assert(last);
  assert(Array.isArray(last.content));
...
test('separate workers with identical prompts create isolated runs', async (t) => {
  const fake = runtime(async (config) => {
    await customTools(config).Read.execute({}, { toolCallId: 'same-sdk-id' });
    return 'done';
  });
  const bridge = new CursorBridge(options, fake);
  t.after(() => bridge.close());
...
test('closing the gateway cancels SDK callbacks waiting between HTTP requests', async () => {
  const fake = runtime(async (config) => {
    await customTools(config).Edit.execute({}, {});
    return 'done';
  });
  const bridge = new CursorBridge(options, fake);
...
test('SDK failure after partial text remains an explicit failure', async (t) => {
  const fake = runtime(async (_config, text) => {
    text('Partial');
    await tick();
  ...
  await assert.rejects(
    bridge.handle(body, 'fail', signal(), (name) => {
      events.push(name);
    }),
    /SDK failed/,
  );
...
test('restart and compacted history start from the supplied Claude transcript', async (t) => {
  ...
  const bridge = new CursorBridge(options, fake);
  t.after(() => bridge.close());
  const history: MessagesRequest = {
    ...body,
    messages: [
      { role: 'user', content: 'Compacted summary: fixture has been edited to beta.' },
      {
        role: 'assistant',
        content: [{ type: 'tool_use', id: 'cursor_old', name: 'Edit', input: {} }],
      },
      {
        role: 'user',
        content: [
          { type: 'tool_result', tool_use_id: 'cursor_old', content: 'Already edited to beta' },
        ],
      },
    ],
  };
...
test('validation rejects unsupported features and counts without SDK inference', () => {
  ...
  for (const invalid of [
    { ...body, model: 'multi/cursor/missing' },
...
test('moving Claude cache markers does not invalidate a tool continuation; edited history does', async (t) => {
  const fake = runtime(async (config) => {
    await customTools(config).Read.execute({}, { toolCallId: 'read' });
    return 'done';
  });
  const bridge = new CursorBridge(options, fake);
  t.after(() => bridge.close());
  const request: MessagesRequest = {
    ...
    messages: [
      {
        role: 'user',
        content: [{ type: 'text', text: 'Read', cache_control: { type: 'ephemeral' } }],
      },
    ],
  };
  ...
  delete content[0].cache_control;
  const edited = structuredClone(next);
  const changed = edited.messages[0].content;
  assert(Array.isArray(changed));
  changed[0].text = 'Different branch';
...
test('duplicate SDK callback IDs reuse a single native tool request', async (t) => {
  let results: unknown[] = [];
  const fake = runtime(async (config) => {
    const tool = customTools(config).Read;
    results = await Promise.all([
      tool.execute({}, { toolCallId: 'same' }),
      tool.execute({}, { toolCallId: 'same' }),
    ]);
    return 'done';
  });
...
test('tool_choice none prevents every SDK execution tool, and prompt images stay attachments', async (t) => {
  ...
  assert.deepEqual(Object.keys(customTools(fake.configs[0])), []);
...
test('long tool names and image/error results survive the callback boundary', async (t) => {
  ...
  const fake = runtime(async (config) => {
    returned = await Object.values(customTools(config))[0].execute({}, { toolCallId: 'image' });
    return 'done';
  });
  const bridge = new CursorBridge(options, fake);
  t.after(() => bridge.close());
...
test('Cursor HTTP route keeps Claude credentials away from SDK and emits Messages SSE', async (t) => {
  ...
  const server = createNativeGateway({
    ...
    cursor: bridge,
    fetchImpl: async () => {
      throw new Error('Cursor must not use OpenAI or Anthropic fetch');
    },
  });
  ...
  const request = (pathname: string, payload: unknown) =>
    fetch(`http://127.0.0.1:${address.port}${pathname}`, {
      method: 'POST',
      headers: { 'x-multi-gateway-token': 'local', authorization: 'Bearer claude-secret' },
  ...
  assert.equal(fake.configs.length, 0);
  assert.deepEqual(await count.json(), { input_tokens: bridge.validate(body) });
  const response = await request('/v1/messages', { ...body, stream: true });
  assert.equal(response.status, 200);
  const sse = await response.text();
  assert(sse.includes('event: message_stop'));
  assert(sse.includes('Hello'));
  assert(!JSON.stringify(fake.configs).includes('claude-secret'));
});

test('Cursor response counts the submitted wrapper and schemas, not cumulative SDK spend', async (t) => {
  const fake = runtime(async () => 'Done', {
    totalTokens: 3000000,
    inputTokens: 900000,
    outputTokens: 800000,
  ...
  const bridge = new CursorBridge(options, fake);
  t.after(() => bridge.close());
  const result = await bridge.handle(body, 'usage', signal());
  const prompt = fake.prompts[0];
  assert(typeof prompt !== 'string');
  const expected =
    estimateTextTokens(prompt.text ?? '') +
    estimateTextTokens(JSON.stringify(customTools(fake.configs[0])));
  assert.equal(bridge.validate(body), expected);
  assert.equal(result.usage.input_tokens, expected);
  assert.equal(result.usage.output_tokens, estimateTextTokens(JSON.stringify(result.content)));
  assert(expected > estimateTextTokens('Read then edit fixture.txt'));
});

test('Cursor token estimates shrink after compaction and include image allowances', async (t) => {
  const fake = runtime(async () => 'Done');
  const bridge = new CursorBridge(options, fake);
  t.after(() => bridge.close());
  const expanded: MessagesRequest = {
    ...body,
    messages: [{ role: 'user', content: 'Disposable context detail. '.repeat(2000) }],
  };
  assert(bridge.validate(expanded) > bridge.validate(body) + 5000);
    ...
    messages: [
      {
        role: 'user',
        content: [
          { type: 'text', text: 'Read then edit fixture.txt' },
          { type: 'image', source: { type: 'base64', media_type: 'image/png', data: 'cGl4ZWxz' } },
        ],
      },
    ],
  };
  assert(bridge.validate(withImage) >= bridge.validate(body) + 4096);
  assert(bridge.validate({ ...body, tool_choice: { type: 'none' } }) < bridge.validate(body));
});
