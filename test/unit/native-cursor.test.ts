...
import type { CreateCursorAgent } from '../../plugins/multi/src/lib/native-cursor.ts';
import { cursorModelOptions, cursorPickerOptions, cursorSelection } from '../../plugins/multi/src/lib/native-cursor-models.ts';
import { createNativeGateway } from '../../plugins/multi/src/lib/native-gateway.ts';
import type { MessagesRequest, MessagesResponse, ResponseContentBlock, Emit } from '../../plugins/multi/src/lib/native-responses.ts';
...
test('Cursor picker defaults to Auto, Grok and Composer, excluding other providers and presets', () => {
  const ids = ['gemini-3.8-flash', 'composer-2', 'claude-fable-5-1', 'default', 'gpt-5.6-sol', 'grok-4.6', 'composer-2.5'];
  const catalog = cursorModelOptions(ids.map(id => ({ id, displayName: id === 'default' ? 'Auto' : id, variants: id === 'default' ? [
  ...
  assert.deepEqual(picker.map(o => o.selection.id), ['default', 'grok-4.6', 'composer-2.5']);
  ...
  assert.deepEqual(catalog, before, 'Picker filtering must not remove full model routes or workers');
  assert.deepEqual(cursorPickerOptions(catalog.filter(o => o.selection.id === 'composer-2.5')).map(o => o.selection.id), ['composer-2.5']);
...
test('extra Cursor picker models are opt-in, deduplicated and checked against the account catalog', () => {
  const catalog = cursorModelOptions(['default', 'grok-4.6', 'composer-2.5', 'gemini-3.8-flash', 'gpt-5.6-sol']
    .map(id => ({ id, displayName: id })));
  const before = structuredClone(catalog);
  assert.deepEqual(cursorPickerOptions(catalog, ' gemini-3.8-flash, gpt-5.6-sol,gemini-3.8-flash,grok-4.6, ')
    .map(o => o.selection.id), ['default', 'grok-4.6', 'composer-2.5', 'gemini-3.8-flash', 'gpt-5.6-sol']);
  assert.deepEqual(cursorPickerOptions(catalog, ' , '), cursorPickerOptions(catalog));
  assert.throws(() => cursorPickerOptions(catalog, 'missing-model'), /MULTI_CURSOR_EXTRA_MODELS.*missing-model.*--cursor-models/);
