...
import {
  ...
  parseJsonResult,
  streamToolKind,
} from '../../plugins/multi/src/transports/cursor.mjs';
...
test('mapStreamEventToProgress: tool_call started → phase; assistant → message_chunk; result → null', () => {
  const phase = mapStreamEventToProgress({
    type: 'tool_call',
    subtype: 'started',
  ...
  const chunk = mapStreamEventToProgress({
    type: 'assistant',
  ...
  const events = [
    {
      type: 'tool_call',
      subtype: 'completed',
      tool_call: { writeToolCall: { args: { path: 'a.js' } } },
    },
    {
      type: 'tool_call',
      subtype: 'completed',
      tool_call: { applyPatchToolCall: { result: { success: { path: 'b.js' } } } },
    },
    {
      type: 'tool_call',
      subtype: 'completed',
      tool_call: { deleteToolCall: { args: { path: 'c.js' } } },
    },
    {
      type: 'tool_call',
      subtype: 'completed',
      tool_call: { readToolCall: { args: { path: 'ignored.js' } } },
    },
    {
      type: 'tool_call',
      subtype: 'completed',
...
test("deriveFileChanges: search_replace edits are captured (regression — not vetoed by 'search')", () => {
  const events = [
    ...
    },
    {
      type: 'tool_call',
      subtype: 'completed',
      tool_call: { webSearchToolCall: { args: { query: 'node version' } } },
    },
    {
      type: 'tool_call',
      subtype: 'completed',
...
test('deriveCommandExecutions: only completed shell tool calls', () => {
  const events = [
    ...
    },
    {
      type: 'tool_call',
      subtype: 'started',
      tool_call: { shellToolCall: { args: { command: 'ignored (started)' } } },
    },
    {
      type: 'tool_call',
      subtype: 'completed',
...
test('normalizeHeadlessOutcome: is_error:true yields an error and status 1', () => {
  const stdoutText =
    '{"type":"result","subtype":"error","is_error":true,"result":"boom","session_id":"s2"}';
  const out = normalizeHeadlessOutcome({ stdoutText, exitCode: 0, outputFormat: 'json' });
...
test('normalizeHeadlessOutcome: stream-json picks the final result event and derives activity', () => {
  const events = [
    { type: 'system', subtype: 'init', session_id: 's3', model: 'Auto' },
    {
      type: 'tool_call',
      subtype: 'completed',
      tool_call: { writeToolCall: { args: { path: 'x.js' } } },
    },
    {
      type: 'tool_call',
      subtype: 'completed',
