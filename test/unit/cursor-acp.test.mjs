...
function tmpRecord() {
  return path.join(
    os.tmpdir(),
    `cur-acp-rec-${process.pid}-${Math.random().toString(36).slice(2)}.jsonl`,
  );
}
...
function withTransport(value, fn) {
  ...
  if (value === undefined) {
    delete process.env.MULTI_TRANSPORT_CURSOR;
  } else {
    process.env.MULTI_TRANSPORT_CURSOR = value;
  }
  return Promise.resolve()
    ...
    .finally(() => {
      if (prev === undefined) {
        ...
        process.env.MULTI_TRANSPORT_CURSOR = prev;
      }
    });
}
...
test('invokeCursor defaults to the headless path when no flag is set', async () => {
  await withTransport(undefined, async () => {
    ...
    process.env.CURSOR_AGENT_PATH = path.join(
      os.tmpdir(),
      'definitely-not-a-real-cursor-agent.cmd',
    );
    try {
      ...
      assert.ok(
        !/ACP runtime not found/i.test(res.error.message ?? ''),
        'did not take the ACP path',
      );
    } finally {
      if (prevPath === undefined) {
        delete process.env.CURSOR_AGENT_PATH;
      } else {
        process.env.CURSOR_AGENT_PATH = prevPath;
      }
    }
  });
});
...
test("resolveCursorModel: unique prefix match on the segment before '['", () => {
  assert.deepEqual(resolveCursorModel(COMPOSITE_IDS, 'composer-2.5'), {
    value: 'composer-2.5[fast=true]',
  });
...
test('ACP path: prefix-resolved model is pinned via set_config_option with the composite id', async () => {
  ...
  try {
    ...
    assert.equal(
      ...
      'composer-2.5[fast=true]',
      'the exact composite id is pinned',
    );
  } finally {
    fs.rmSync(rec, { force: true });
  }
...
test('ACP path: an unresolvable model fails before prompting with the available list', async () => {
  ...
  try {
    ...
    assert.ok(
      ...
      'no prompt sent on an unresolvable model',
    );
  } finally {
    fs.rmSync(rec, { force: true });
  }
