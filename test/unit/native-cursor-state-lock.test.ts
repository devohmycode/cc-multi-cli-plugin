...
test('kernel lock excludes concurrent owners and releases after a gateway crash', {
  ...
  const child = spawn(
    ...
    [
      '--input-type=module',
      '-e',
      `import {lockCursorSession} from ${JSON.stringify(module.href)};const release=await lockCursorSession(process.argv[1]);process.stdout.write('ready');process.stdin.on('end',release);process.stdin.resume();`,
      file,
    ],
  ...
  await assert.rejects(lockCursorSession(file), /locked/);
  child.kill('SIGKILL');
  await exited;
  let release: (() => Promise<void>) | undefined;
  for (let attempt = 0; attempt < 50 && !release; attempt++) {
    try {
      release = await lockCursorSession(file);
    } catch {
...
test('legacy directory locks remain explicit recovery evidence', async (t) => {
  ...
  const file = path.join(directory, 'session.lock');
  await mkdir(file);
  await assert.rejects(lockCursorSession(file), /legacy interrupted lock/);
});
...
test('missing flock fails explicitly and closes the opened descriptor', async (t) => {
  ...
  try {
    await assert.rejects(lockCursorSession(path.join(directory, 'session.lock')), /ENOENT/);
    assert.equal((await readdir('/proc/self/fd')).length, descriptors);
  } finally {
