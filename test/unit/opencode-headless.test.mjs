...
import {
  ...
  resolveModel,
  streamToolKind,
} from '../../plugins/multi/src/transports/opencode.mjs';
...
test('resolveModel: OPENCODE_CLI_DEFAULT_MODEL is used when no explicit model', () => {
  ...
  try {
    ...
    if (prev === undefined) {
      delete process.env.OPENCODE_CLI_DEFAULT_MODEL;
    } else {
      process.env.OPENCODE_CLI_DEFAULT_MODEL = prev;
    }
  }
});
...
test('buildHeadlessArgs: model default + override + blank → default', () => {
  assert.equal(
    buildHeadlessArgs({ role: 'delegate' })[
      buildHeadlessArgs({ role: 'delegate' }).indexOf('--model') + 1
    ],
    'opencode/claude-opus-5',
  );
...
test('buildHeadlessArgs: cwd appends --dir <cwd> (cursor --workspace analog); absent/blank → no --dir', () => {
  ...
  assert.ok(
    !buildHeadlessArgs({ role: 'delegate' }).includes('--dir'),
    'no --dir when cwd omitted',
  );
  assert.ok(
    !buildHeadlessArgs({ role: 'delegate', cwd: '   ' }).includes('--dir'),
    'no --dir for blank cwd',
  );
});
...
test('buildOpencodeSpawnEnv: clears server creds for all roles; injects config+floor for read-only', () => {
  ...
  assert.equal(
    write.OPENCODE_CONFIG_CONTENT,
    undefined,
    'write role must not inject the read-only config',
  );
...
test('mapEventToProgress: tool_use/step_start → phase; text → message_chunk; else null', () => {
  const phase = mapEventToProgress({
    type: 'tool_use',
...
test('deriveFileChanges: completed write/edit/delete tool_use w/ input.filePath → change; reads excluded; deduped', () => {
  const events = [
    {
      type: 'tool_use',
      part: { tool: 'write', state: { status: 'completed', input: { filePath: 'a.js' } } },
    },
    {
      type: 'tool_use',
      part: { tool: 'edit', state: { status: 'completed', input: { filePath: 'b.js' } } },
    },
    {
      type: 'tool_use',
      part: { tool: 'delete', state: { status: 'completed', input: { filePath: 'c.js' } } },
    },
    {
      type: 'tool_use',
      part: { tool: 'read', state: { status: 'completed', input: { filePath: 'ignored.js' } } },
    },
    {
      type: 'tool_use',
      part: { tool: 'write', state: { status: 'completed', input: { filePath: 'a.js' } } },
...
test('deriveFileChanges: only completed status counts; pending/error excluded', () => {
  const events = [
    {
      type: 'tool_use',
      part: { tool: 'write', state: { status: 'pending', input: { filePath: 'p.js' } } },
    },
    {
      type: 'tool_use',
      part: { tool: 'write', state: { status: 'error', input: { filePath: 'e.js' } } },
    },
    {
      type: 'tool_use',
      part: { tool: 'write', state: { status: 'completed', input: { filePath: 'ok.js' } } },
    },
  ];
  ...
  const events = [
    {
      ...
      part: {
        tool: 'search_replace',
        state: { status: 'completed', input: { filePath: 'edited.js' } },
      },
    },
    {
      type: 'tool_use',
      part: { tool: 'websearch', state: { status: 'completed', input: { query: 'node version' } } },
    },
    {
      type: 'tool_use',
      part: { tool: 'grep', state: { status: 'completed', input: { pattern: 'foo' } } },
    },
  ];
...
test('deriveCommandExecutions: only completed bash tool_use', () => {
  const events = [
    {
      type: 'tool_use',
      part: { tool: 'bash', state: { status: 'completed', input: { command: 'npm test' } } },
    },
    {
      type: 'tool_use',
      part: { tool: 'bash', state: { status: 'pending', input: { command: 'ignored (pending)' } } },
    },
    {
      type: 'tool_use',
      part: { tool: 'read', state: { status: 'completed', input: { filePath: 'x' } } },
    },
  ];
...
test('firstSessionId: returns the first sessionID seen', () => {
  ...
  assert.equal(
    firstSessionId([{ type: 'step_start' }, { type: 'text', sessionID: 'ses_2' }]),
    'ses_2',
  );
...
test('normalizeHeadlessOutcome: concats text parts, sessionId from first sessionID, step_finish:stop → status 0', () => {
  const events = [
    { type: 'step_start', sessionID: 'ses_x' },
    {
      type: 'tool_use',
      sessionID: 'ses_x',
      part: { tool: 'write', state: { status: 'completed', input: { filePath: 'x.js' } } },
    },
    {
      type: 'tool_use',
      sessionID: 'ses_x',
      part: { tool: 'bash', state: { status: 'completed', input: { command: 'echo hi' } } },
    },
...
test('normalizeHeadlessOutcome: exit 0 but no text and no stop → treated as error', () => {
  const out = normalizeHeadlessOutcome({
    events: [{ type: 'step_start', sessionID: 's' }],
    exitCode: 0,
    role: 'delegate',
  });
  assert.equal(out.status, 1);
