...
function tmpRecord() {
  return path.join(
    os.tmpdir(),
    `oc-acp-rec-${process.pid}-${Math.random().toString(36).slice(2)}.jsonl`,
  );
}
...
function withTransport(value, fn) {
  ...
  if (value === undefined) {
    delete process.env.MULTI_TRANSPORT_OPENCODE;
  } else {
    process.env.MULTI_TRANSPORT_OPENCODE = value;
  }
  return Promise.resolve()
    ...
    .finally(() => {
      if (prev === undefined) {
        ...
        process.env.MULTI_TRANSPORT_OPENCODE = prev;
      }
    });
}
...
test('invokeOpencode defaults to the headless path when no flag is set', async () => {
  ...
  await withTransport(undefined, async () => {
    ...
    process.env.OPENCODE_CLI_PATH = path.join(
      os.tmpdir(),
      'definitely-not-a-real-opencode-binary.cmd',
    );
    try {
      ...
      assert.ok(
        !/ACP executable not found/i.test(res.error.message ?? ''),
      ...
      if (prevPath === undefined) {
        ...
        process.env.OPENCODE_CLI_PATH = prevPath;
      }
    }
  });
});
...
test('ACP path: the model is set via set_config_option (provider/model id passes through)', async () => {
  ...
  try {
    const res = await runAcpOpencodeTurn(repoRoot, 'hi', {
      ...
      spawnSpec: fixtureSpawnSpec([
        '--models',
        'opencode/claude-opus-4-8,opencode/gpt-5-nano',
        '--record',
        rec,
        '--stream',
        'ok',
...
test('ACP path: a model not in the live list fails before prompting (no silent fallback)', async () => {
  ...
  try {
    ...
    assert.ok(
      ...
      'no prompt sent on an invalid model',
    );
  } finally {
    fs.rmSync(rec, { force: true });
  }
