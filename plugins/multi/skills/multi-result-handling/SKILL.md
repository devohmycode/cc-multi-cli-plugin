...
When a `multi:*` subagent returns helper output (Codex, Cursor, OpenCode via multi, or any CLI added via `multi-cli-anything`):
...
For research, review, or diagnosis output from `/codex:review`, `multi:cursor-research`, `multi:opencode-researcher`, `multi:opencode-explorer`, etc.:
