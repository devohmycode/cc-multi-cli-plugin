...
function wireIdentifier(value: string, prefix: string): string {
  if (typeof value !== 'string' || !value.trim()) throw new Error('Expected a nonempty tool identifier');
  if (/^[a-zA-Z0-9_-]{1,64}$/.test(value) && !value.startsWith(prefix)) return value;
...
export function originalToolNames(body: MessagesRequest): Map<string, string> {
  ...
  const add = (name: string) => {
    ...
    if (names.has(alias) && names.get(alias) !== name) throw new Error('Tool alias collision');
    names.set(alias, name);
  };
  for (const tool of body.tools ?? []) if (tool.name) add(tool.name);
  return names;
}
