...
export function terminateProcessTree(pid, options = {}) {
  ...
  if (platform === 'win32') {
    return terminateWindowsProcessTree(pid, options, runCommandImpl, killImpl);
  }
...
function terminateWindowsProcessTree(pid, options, runCommandImpl, killImpl) {
...
function resolveWindowsCommand(command, env = process.env) {
  ...
  const result = {
    command: resolved.replace(/\\/g, '/'),
    extension: getWindowsExtension(resolved),
  };
