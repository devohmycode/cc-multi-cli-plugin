...
const child = spawn(process.execPath, [launcher, '--', '-p', '--input-format', 'stream-json', '--output-format', 'stream-json', '--verbose',
  ...
  CLAUDE_CODE_MAX_RETRIES: '0' }, stdio: ['pipe', 'pipe', 'pipe'] });
const events: ClaudeEvent[] = [], waiters: Waiter[] = [];
let diagnostics = '';
...
lines.on('line', line => {
  ...
  events.push(event);
  for (const waiter of [...waiters]) if (waiter.match(event)) {
    waiters.splice(waiters.indexOf(waiter), 1); clearTimeout(waiter.timer); waiter.resolve(event);
  }
});
const wait = (match: (event: ClaudeEvent) => boolean) => new Promise<ClaudeEvent>((resolve, reject) => {
  const waiter = { match, resolve, timer: setTimeout(() => reject(new Error('Live switch test timed out')), 120000) };
  waiters.push(waiter);
});
const send = (value: unknown) => child.stdin.write(JSON.stringify(value) + '\n');
async function control(request: { subtype: string; model?: string }) {
  const request_id = randomUUID();
  const pending = wait(event => event.type === 'control_response' && event.response?.request_id === request_id);
...
async function turn(content: string) {
  ...
  console.log(JSON.stringify({ result: result.result, subagents: result.subagent_stats?.completed }));
  return result;
}
try {
  await control({ subtype: 'initialize' });
  const first = await turn('Read fixture.txt with Read. Remember its exact contents for later and report them. Do not edit it.');
  assert(first.result?.includes(nonce));
  await control({ subtype: 'set_model', model: 'multi/openai/gpt-5.6-luna' });
  const second = await turn('Without rereading fixture.txt, use Edit to replace alpha with beta, preserving the nonce you saw earlier. Report that nonce.');
  assert(second.result?.includes(nonce));
  assert.equal(await readFile(path.join(cwd, 'fixture.txt'), 'utf8'), `beta ${nonce}\n`);
  const delegated = await turn('Delegate to openai-luna-low: Read fixture.txt and report its exact contents. Do not read it yourself. Wait for the worker and return its result.');
  ...
  const last = await turn('Without any tools, report the original and current contents of fixture.txt from our conversation.');
  assert(last.result?.includes(nonce) && last.result.includes('alpha') && last.result.includes('beta'));
  assert(events.some(event => event.type === 'assistant' && event.message?.model === 'multi/openai/gpt-5.6-luna'));
  assert(diagnostics.includes('"route":"anthropic","status":200'));
  console.log('PASS: Claude → GPT main → native delegation → Claude, preserving text and tool history.');
} finally {
